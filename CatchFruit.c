...
void drawapple (applenode *, bombnode *, dianode *);
...
gradient (void)
{
  int red = 125, green = 140, blue = 255, i, j;

  for (i = 0, j = 0; i < 618; i = i + 6, j++)
    {
      gfx_color (red, green + j, blue);
      gfx_line (0, i, 630, i);
      gfx_line (0, i + 1, 630, i + 1);
      gfx_line (0, i + 2, 630, i + 2);
      gfx_line (0, i + 3, 630, i + 3);
      gfx_line (0, i + 4, 630, i + 4);
      gfx_line (0, i + 5, 630, i + 5);
      gfx_flush ();
    }
}
...
resetapple (applenode * p, int *applepointer)
{
  ...
  p->velocity = rand () % 7 + 1;	// re-randomize velocity
...
resetdia (dianode * r)
{
  ...
  r->dypos = 25;		// reset apple to start from top
  ...
  for (n = 1; n <= 4; n++)
    {
      ...
      p->ypos = 20;
      p->velocity = rand () % 5 + rand () % 5 + 1;	// randomize velocity
      p->next = head;
...
void
drawapple (applenode * p, bombnode * q, dianode * r)
{
  int i;
  while (p)
    {
      ...
      for (i = 1; i <= 25; i++)
	{
	  gfx_circle (p->xpos, p->ypos, i);	// create circle
	}
  ...
  while (q)
    {
      ...
      for (i = 1; i <= 20; i++)
	{
	  gfx_circle (q->bxpos, q->bypos, i);	// create circle
	}
      gfx_color (0, 0, 0);
